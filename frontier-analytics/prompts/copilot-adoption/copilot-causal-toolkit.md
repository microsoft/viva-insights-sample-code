...
See the [Copilot Causal Toolkit documentation](https://microsoft.github.io/viva-insights-sample-code/copilot-causal-toolkit/) for the full methodology behind this prompt.

## Audience

HR analytics leads, people analytics practitioners who want to evaluate the causal impact of Copilot on business outcomes (for example seller productivity, employee wellbeing, engagement).
...
2. Load the data using `import_query()` from the `vivainsights` package. Import the package (`import vivainsights as vi`) and then call `data = vi.import_query("path/to/data.csv")`.
...
Ask the user which business question they want to answer:
  a) "Does Copilot usage increase time spent with external customers/partners?" leads to ExtCollabHours
  b) "Does Copilot usage reduce after-hours work and burnout risk?" leads to AftCollabHours
  c) "Does Copilot usage improve employee engagement survey scores?" leads to Engagement (PQ only)
...
2. ORGANIZATIONAL ATTRIBUTES: Update `SUBGROUP_VARS` to match the HR attribute columns identified by `extract_hr()` in Step 2. Include 2-4 key attributes (for example Organization, FunctionType, LevelDesignation, IsManager). Only include attributes that actually exist in the data.
...
4. ANALYSIS DIRECTION (AftCollabHours notebooks only): Set `FIND_NEGATIVE_EFFECTS`:
   - True finds subgroups where Copilot reduces after-hours work (typical use case)
   - False finds subgroups where Copilot increases after-hours work
...
- Keep explanations accessible, since the user may not be a data scientist.
```

### Adaptation notes for Prompt 1

- **Multiple scenarios:** If the user wants to run more than one scenario (for example both seller productivity and burnout prevention), run them sequentially. Each notebook generates its own output folder.
...
- "Copilot usage is associated with a statistically significant increase (or decrease) in [outcome] of [X hours/points] per person per week, after controlling for other factors."
...
- Which group of employees is this? (for example "Managers in Engineering")
- What is the estimated effect? (for example "1.2 fewer after-hours collaboration hours per week")
- How confident are we? Reference the sensitivity analysis. Higher E-values mean the finding is more robust to hidden biases.
- What does this mean practically? (for example "This suggests that Copilot helps managers in Engineering reduce after-hours work by roughly 1 hour per week.")
...
- Avoid statistical jargon. Translate p-values, confidence intervals, and E-values into plain language (for example "We are 95% confident the true effect is between X and Y").
...
- If no significant effects are found, this is still a valid and useful result. Frame it constructively (for example "The data does not yet show a measurable effect, which may indicate the need for a longer observation period or targeted enablement strategies").
```

### Adaptation notes for Prompt 2

- **Audience customization:** If the audience is more technical (for example data science peers), you can ask the agent to include more statistical detail (confidence intervals, model diagnostics, covariate balance checks).
- **Multiple scenarios:** If the user ran multiple notebooks (for example both ExtCollabHours and AftCollabHours), ask the agent to synthesize findings across all scenarios into a single interpretation document.
- **Visualization requests:** You can ask the agent to generate additional charts from the output CSVs (for example a forest plot of subgroup effects, or a bar chart comparing effect sizes across organizational segments).
...
- **Agent ignores sensitivity analysis.** The E-values are critical for assessing robustness. If the agent skips this section, prompt it: "Also interpret the sensitivity analysis results from the JSON file."
