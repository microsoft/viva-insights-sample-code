...
Frontier Analytics is a folder (`frontier-analytics/`) containing reusable assets for self-service analytics on exported Viva Insights data. It is not a standalone tool or application. The assets — prompt cards, starter kits, schema docs, and templates — are designed to be used _with_ a coding agent to generate dashboards, reports, and analysis notebooks.
...
Browse the [starter kits](starter-kits/) to find a workflow that matches your use case. Each kit bundles a use case, required inputs, prompts, and expected outputs.

| Starter kit | Description | Complexity | Output |
|-------------|-------------|------------|--------|
| [Copilot Adoption Dashboard](starter-kits/copilot-adoption-dashboard/) | Interactive HTML dashboard tracking Copilot adoption metrics, usage trends, and user segmentation | Intermediate | HTML dashboard |
| [Executive Summary Report](starter-kits/executive-summary-report/) | One-page memo summarizing key collaboration and Copilot metrics for leadership | Beginner | Markdown / HTML memo |

If no starter kit fits your scenario, browse the [prompt card library](prompts/) for individual analysis prompts.

### 2. Prepare your data files

- Export the required query from the Viva Insights Analyst portal (the starter kit README specifies which query types).
...
> **Tip:** Missing values in person query data may indicate unlicensed users. Check the [schemas documentation](schemas/) for details on how to handle gaps.

### 3. Review the schema docs

Read the relevant pages in [schemas/](schemas/) to understand:
...
Each starter kit references specific prompt cards from the [prompts/](prompts/) library. Open those cards and read through:
...
- [Starter kits overview](STARTER_KITS.md) — full list of available kits
